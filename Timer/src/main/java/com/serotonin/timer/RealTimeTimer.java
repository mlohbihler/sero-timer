...
public class RealTimeTimer extends AbstractTimer {
    private static final Logger LOGGER = LoggerFactory.getLogger(RealTimeTimer.class);
        ...
        synchronized (queue) {
            if (!thread.newTasksMayBeScheduled) {
                if (cancelStack != null) {
                    ...
                    LOGGER.error("   Cancel stack:", cancelStack);
                    LOGGER.error("   Current stack:", new Exception());
            ...
            synchronized (task.lock) {
                if (task.state == TimerTask.VIRGIN) {
                    final long time = task.trigger.getFirstExecutionTime();
    ...
    public List<TimerTask> getTasks() {
        final List<TimerTask> result = new ArrayList<>();
        synchronized (queue) {
